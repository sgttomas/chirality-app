...
Outputs:
    ...
    manifest.json — metadata: pdf_path, pdf_sha256, page_count, dpi,
                    pages_rendered, files. The pdf_sha256 field closes
                    the "same-path PDF replacement" gap noted by
                    AGENT_PDF2MD.md — Phase 0 can now detect when the
                    PDF behind a given path has changed identity.
...
def file_sha256(path, chunk_size=1 << 20):
    """Stream-compute SHA256 of a file (1 MB chunks; safe for large PDFs)."""
    h = hashlib.sha256()

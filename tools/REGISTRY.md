...
| `sanitize_equipment_stubs.py` | Python 3 | Target-aware post-extraction QC on v2 stubs (instrument/note-name filters on base columns; detail columns pass through unchanged) | --source-dir, --drawing-type, --extraction-target, --pdf-stem, --start-page, --end-page, [--report-csv] | Sanitized v2 stub `.md` files in-place + optional QC report CSV |
| `report_stub_counts.py` | Python 3 | Target-aware per-page coverage report from v2 stubs. Basic: 7-col status CSV. Detailed: per-field populated/total counts + required-field warnings + identical-value heuristic (possible copy-across hallucination flag) | --source-dir, --drawing-type, --extraction-target, --pdf-stem, --start-page, --end-page, --output-csv | Coverage report CSV |

...
| Name | Language | Purpose | Inputs | Outputs |
|------|----------|---------|--------|---------|
| `build_section_map.py` | Python 3 | First publication step: generate candidate `Section_Map.csv` from approved `Publication_Schema.md` selectors and the frozen `Publication_Input_Manifest.md`, validating selector coverage, duplicate artifact rows, and section load against `MaxKAFiles` without interpreting free-text inclusion/exclusion prose | `--schema`, `--manifest`, `--output-csv`, `--report-md` | Candidate `Section_Map.csv` + coverage report markdown |
| `build_concordance_candidates.py` | Python 3 | Second publication step: read the frozen `Publication_Input_Manifest.md`, approved `Publication_Schema.md`, and approved `Section_Map.csv`, harvest typed concordance candidates from structured sources (mapped KA tables, open issues, decision log, active SCA amendment CSVs), and write an agent-seedable candidate register plus a coverage summary without doing prose-only inference | `--manifest`, `--schema`, `--section-map`, `--output-csv`, `--coverage-md` | `Publication_Concordance_Candidates.csv` + `Publication_Concordance_Coverage.md` |
| `render_dispatch_briefs.py` | Python 3 | Third publication step: read the frozen planning artifacts plus skill brief schemas, render deterministic INIT-TASK briefs for approved publication sections and the package gate, validate each rendered brief against the target skill `BRIEF_SCHEMA.md`, pre-create section/package output directories, and emit `DISPATCH_INDEX.csv`; supports targeted section rerender via explicit SectionID list and does not write section/package publication outputs | `--publication-root`, `--input-manifest`, `--schema`, `--section-map`, `--rules`, `--concordance-register`, `--dispatch-root`, `--sections-root`, `--package-root`, `--package-snapshot-name`, `[--skills-root]`, `[--section-ids]`, `[--render-package yes|no]` | Section/package INIT-TASK briefs + `DISPATCH_INDEX.csv` + directory scaffolding |
| `assemble_publication.py` | Python 3 | Fourth publication step: read publication-root planning artifacts plus approved section outputs, assemble package-level publication artifacts, run deterministic completeness/trace reporting, and optionally update `_LATEST.md`; writes only the package snapshot outputs in `--output-dir` plus optional `_LATEST.md` and returns exit `2` when blocking completeness/trace findings remain | `--publication-root`, `--input-manifest`, `--schema`, `--section-map`, `--rules`, `--sections-root`, `--output-dir`, `[--latest-pointer]` | `Rewritten_DBM.md`, `Trace_Appendix.md`, `Publication_Manifest.md`, `Publication_QA.md`, optional `_LATEST.md` |
| `check_concordance.py` | Python 3 | Final deterministic publication gate: read the frozen `Publication_Concordance_Register.csv` plus current section assertion CSVs, perform deterministic cross-section concordance checks, and write only the concordance report/findings outputs; returns exit `2` when blocking findings remain | `--register`, `--sections-root`, `--output-report`, `--output-findings` | `Publication_Concordance_Report.md` + `Publication_Concordance_Findings.csv` |

...
|----------|---------------------|
| Datasheet.md | Identification section defines data sheet attributes (numbers, equipment tags, service description, classification); Conditions section lists design context (service application, product, throughput, metering points) and all design basis parameters with TBD sources; Construction section lists anticipated data sheet content (flow meters ×2, temperature transmitters, pressure transmitters) and defines data sheet field categories (Identification, Process Conditions, Performance, Materials, Electrical/Controls, Physical, Calibration/Proving, Vendor, Verification Basis) matching field requirements in this Specification |
| Guidance.md | Purpose section explains data sheets serve as structured, auditable source of truth for equipment parameters linking specification to procurement, drawings, calculations, and QA records (per REQ-05, REQ-06, REQ-11); Principles section explains data sheet development rationale (single source of truth, traceability per REQ-10, consistency per REQ-04/REQ-05/REQ-06) and custody transfer principles (accuracy capture per REQ-07, calibration traceability per REQ-11, proving fields); Considerations section identifies content considerations (service differentiation per REQ-02, vendor data timing, interface fields per REQ-13, calibration fields per REQ-11), flow meter field considerations, transmitter field considerations |
| Procedure.md | Steps section defines data sheet development workflow aligned to requirements: Step 1 (Identify required data sheets per REQ-01; assign tag numbers per REQ-02/REQ-04), Step 2 (Establish data sheet template with field categories per field requirements above), Step 3 (Populate design basis from ER, DEL-12.02, DEL-12.03 per REQ-05, REQ-06; document verification basis per REQ-10), Step 4 (Obtain and incorporate vendor data; mark vendor-certified fields per REQ-10), Step 5 (Cross-check consistency per REQ-04, REQ-05, REQ-06), Step 6 (Independent check per REQ-15 and issue); Verification section confirms requirement satisfaction per Acceptance Criteria; Records section identifies data sheet outputs matching Documentation section |

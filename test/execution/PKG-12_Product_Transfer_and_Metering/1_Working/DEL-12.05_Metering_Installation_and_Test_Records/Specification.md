...
|----------|---------|---------------------|
| Datasheet.md | § Identification | DEL-12.05 identity, discipline (Process), type (Record), responsible party (D&B Contractor QA/QC) |
| Datasheet.md | § Attributes | Record package attributes (number TBD, category, retention period TBD, revision, classification, record count) |
| Datasheet.md | § Conditions | Design context (service application, record scope, equipment covered per REQ-01, services); record requirements context (calibration standard per REQ-06, verification method per REQ-08, proving frequency, acceptance criteria per REQ-05/09, witnessing per REQ-16, archive format per REQ-17 — all TBD sources) |
| Datasheet.md | § Construction | Anticipated record content (calibration certificates per REQ-01/06/07, accuracy verification per REQ-01/05/08) matching § Documentation; record package structure (cover sheet, record index per REQ-04, sections 1-8, appendices per REQ-12) matching Documentation section; equipment-to-record traceability per REQ-02/10/11 |
| Guidance.md | § Purpose | Records as auditable evidence (proper specification REQ-05, installation per DEL-12.01, calibration per REQ-06, verification per REQ-05); downstream use (closeout, regulatory per REQ-06, commercial, operational, audit, warranty) |
| Guidance.md | § Principles | Development rationale: evidence (REQ-05), traceability (REQ-02/03/06/10/11), auditability (REQ-17), regulatory (REQ-06 Measurement Canada), no fabrication; custody transfer principles: calibration traceability (REQ-06), proving documentation (REQ-08), witnessing (REQ-16) |
| Guidance.md | § Considerations | Content: minimum per Decomposition:360 (REQ-01), equipment coverage (REQ-01), acceptance per REQ-05, witnessing per REQ-16; calibration certificate fields per REQ-07; proving record fields per REQ-08; service-specific (rail batch, marine export); timing (FAT through handover) |
| Guidance.md | § Trade-offs | Proving depth vs. schedule (REQ-05), vendor vs. project format (REQ-07), witnessing vs. cost (REQ-16), comprehensive vs. minimum records (REQ-01) |
| Procedure.md | § Step 1 | Define record set: minimum per REQ-01 (Decomposition:360), ER/DEL-12.02/Quality Plan additions, equipment from DEL-12.04 per REQ-02/03, witnessing per REQ-16 |
| Procedure.md | § Step 2 | Traceability map (record index) per REQ-04: equipment list from DEL-12.04 per REQ-03; index columns per REQ-04 (tag, serial, service, type, date, reference, witness, status) |
| Procedure.md | § Step 3 | Calibration certificates: collect per REQ-01, verify completeness per REQ-07, verify traceability per REQ-06, cross-reference per REQ-03/10 |
| Procedure.md | § Step 4 | Proving records: collect per REQ-01, verify completeness per REQ-08, verify acceptance per REQ-05/09, cross-reference per REQ-03/11 |
| Procedure.md | § Step 5 | Verify: completeness per REQ-01/17, traceability per REQ-02/03/06/10/11, acceptance per REQ-05/09, signatures per REQ-16, NCRs per REQ-18 |
| Procedure.md | § Step 6 | Compile package: structure per § Documentation/REQ-14, cross-references per REQ-12, document control per REQ-13 |
| Procedure.md | § Step 7 | Issue: review, approvals per REQ-16, document control per REQ-13, archive per retention, status update |
| Procedure.md | § Verification | Activities mapped to requirements; acceptance criteria per § Acceptance Criteria |
| Procedure.md | § Records | Outputs: calibration certificates per REQ-01/06/07, proving records per REQ-01/05/08, record index per REQ-04, supporting records (FAT, installation, SAT, MTRs, NCRs per REQ-18)

...
|----------|---------------------|
| Datasheet.md | Attributes section identifies specification attributes (document number, type, revision); Conditions section defines design context (service application, product, throughput, metering points, operating conditions) that specification must address; Construction section lists key specification topics (performance, flow range, materials, installation, proving, instrumentation, electrical/controls, QA) and anticipated specification structure matching requirements in this document |
| Guidance.md | Purpose section explains that this specification drives DEL-12.01 drawings, DEL-12.03 calculations, DEL-12.04 data sheets, and DEL-12.05 QA records; Principles section explains specification development rationale (contract alignment per REQ-01/REQ-02, objective support per REQ-09/REQ-10, testability per REQ-21, traceability per REQ-24) and custody transfer metering principles (accuracy/uncertainty per REQ-06/REQ-10, repeatability per REQ-08, proving per REQ-04/REQ-17/REQ-18/REQ-19, compensation per REQ-03); Considerations section identifies technology selection factors (meter technology per REQ-05, product properties per REQ-11, flow range per REQ-07/REQ-09, proving method per REQ-17), service-specific considerations (rail unloading vs. marine loading per REQ-01), and interface considerations (PKG-14/17/19/20 per REQ-15/REQ-16) |
| Procedure.md | Steps section defines authoring workflow aligned to requirements: Step 1 (Gather basis from ER and standards per Standards section), Step 2 (Define scope per REQ-01), Step 3 (Draft requirements per REQ-03 through REQ-23), Step 4 (Add verification methods per REQ-21 and traceability matrix per REQ-24), Step 5 (Discipline check per Verification Methods), Step 6 (IDC per REQ-16), Step 7 (Issue); Verification section confirms requirement satisfaction per Acceptance Criteria; Records section identifies specification outputs matching Documentation section |

...
subagents: EVALUATION_REPORT, CONTENT_DIGEST, EVALUATION_STRUCTURE_AUDIT, EVALUATION_DEPENDENCY_AUDIT
...
### With CONTENT_DIGEST (Type 2)
- EVALUATION dispatches one CONTENT_DIGEST agent per deliverable folder.
- CONTENT_DIGEST agents write individual digest files to `_Evaluation/content-digests/{PKG-ID}/`.
- EVALUATION may batch dispatches by package for manageability.
- CONTENT_DIGEST agents use Haiku model for efficiency.
...
- They may be dispatched in parallel with CONTENT_DIGEST agents.
...
4. Present the evaluation plan to the human:
   - Number of content digest agents to dispatch
...
3. Dispatch CONTENT_DIGEST agents (batched by package) for all deliverables.
...
The multi-agent architecture (orchestrator + digest agents + report agents) enables parallelism across deliverables and dimensions while keeping each agent's context bounded. Haiku agents handle high-volume, low-reasoning digest work; Sonnet agents handle dimension scoring that requires cross-referencing and judgment.

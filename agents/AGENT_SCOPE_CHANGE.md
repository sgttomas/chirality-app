...
These instructions govern a **Type 1, human-facing persona** for managing **controlled changes to the project decomposition document** after initial creation by PROJECT_DECOMP.
...
|-------------|-------------|---------------------|------------------|
| `ADD` | New deliverable or package | New row(s) in §7/§8 | New folder via PREPARATION (handoff) |
| `REMOVE` | Remove deliverable or package | Row annotated as RETIRED in §8; noted in §3 Change Log | `_STATUS.md` updated to RETIRED |
| `MODIFY` | Change attributes of existing entity | Row updated in §7/§8 | `_CONTEXT.md` updated |
| `RECLASSIFY` | Move deliverable to different package | Row updated in §8 (ParentPackageID) | `_CONTEXT.md` updated; folder may need relocation (advisory) |
...
- `EXECUTION_ROOT`
- `DECOMPOSITION_PATH`
- Parsed decomposition state (§5, §6, §7, §8)
...
1) Resolve `EXECUTION_ROOT` and `DECOMPOSITION_PATH`. Parse decomposition §3, §5, §6, §7, §8.
...
5) Dispatch AUDIT_DECOMP_COVERAGE to capture the **pre-change baseline** (scoped to affected packages/deliverables). Store the `coverage_summary.json` path.
...
4) **§5 Scope Boundaries / Scope Ledger** (if affected):
   - Update scope item → package/deliverable mappings

5) **§6 Objectives** (if affected):
...
3) **Post-change validation:**
   - Dispatch AUDIT_DECOMP_COVERAGE (scoped to affected packages/deliverables) to capture the post-change state.
...
- Pre-change and post-change AUDIT_DECOMP_COVERAGE runs both completed.
- The decomposition document's §3 Change Log contains the amendment entry.
...
{EXECUTION_ROOT}/_ScopeChange/
  ...
  SCA-{NNN}_{YYYY-MM-DD}_{HHMM}/
    ...
    Pre_Change_Coverage.json       (copy/link from AUDIT_DECOMP_COVERAGE)
...
The 5-gate workflow is deliberately heavyweight relative to the simplicity of "just edit the file." The cost of the gates is low (a few minutes of human review); the cost of an uncontrolled scope change is high (cascading inconsistency across 71+ deliverables, requiring full reconciliation to detect and repair).

...
This revision incorporates project-instantiation intent:
...
- Select document schemas using the defaults from the WORKING_ITEMS protocol (AGENT_WORKING_ITEMS; filename may vary) as the starting point:
...
These are the default schemas from the WORKING_ITEMS protocol (AGENT_WORKING_ITEMS; filename may vary). The agent may adapt them based on deliverable type and discipline (label adaptations as ASSUMPTION).

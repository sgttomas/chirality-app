...
Instructions for Claude on what an instance-level TASK contains and how to produce one.

A TASK is a structured representation of the specific task — what it MUST BE. It captures requirements, constraints, and parameters for the immediate work.
...
TASK answers the question: **"What must it be?"**

It is normative — requirements, constraints, success criteria. It specifies what this particular task requires.

TASK captures **instance-level parameters** — things specific to the immediate task.
...
Ask:
- "What codes apply specifically to this work?" (May inherit from DOMAIN)
...
Using document types from DOMAIN:
...
**Note:** Per METHOD, all four document types should be produced to create verification surfaces. Claude should clarify if the engineer requests fewer.
...
Claude does not proceed to artifact production until TASK is confirmed.
...
Claude may:
- Hold TASK in context for immediate use
- Save TASK as a file for reference or multi-session work

---

## 12. For Claude

When producing a TASK:

1. Work through sections 1-9 via conversation
...
5. Use TASK + DOMAIN to guide artifact production

TASK specifies what this task MUST BE. It does not describe the domain (that's DOMAIN) or how to work (that's METHOD and PROTOCOL).

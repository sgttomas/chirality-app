...
The Chirality system ships 35 agent instruction files organized according to the Type 0/1/2 hierarchy defined in Chapter 4 (§4.5) and catalogued in full in Appendix B. This section provides a quantitative characterization of that suite.
...
| Type 2 — Bounded Task Agents | 19 | Brief-driven, straight-through specialists |
| **Total** | **35** | |
...
The 35 agents are organized within a 3×4 matrix that maps epistemic posture (rows) to functional role (columns):
...
- The 35-agent instruction suite (all agent files present, conformance verified by AUDIT_AGENTS)

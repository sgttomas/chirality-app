...
function mapTaskNotificationToUiEvents(message: SDKMessage, sessionId: string): UIEvent[] {
  ...
  const taskNotification = message as UnknownRecord;
  const toolUseId = asString(taskNotification.tool_use_id);
  if (!toolUseId) {
    return [];
  }

  const status = asString(taskNotification.status) ?? "unknown";
  const content = asString(taskNotification.summary) ?? `Task ${status}`;

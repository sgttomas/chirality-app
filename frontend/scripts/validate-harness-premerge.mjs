...
const TOOLKIT_VALIDATION_SCRIPT = path.join(SCRIPT_DIR, "validate-toolkit.mjs");
...
async function runToolkitValidation() {
  const child = spawn(process.execPath, [TOOLKIT_VALIDATION_SCRIPT], {
    cwd: FRONTEND_ROOT,
    env: process.env,
    stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  let status = "";
  const parseLine = (line) => {
    if (line.startsWith("TOOLKIT_VALIDATION_STATUS=")) {
      status = line.slice("TOOLKIT_VALIDATION_STATUS=".length).trim();
    }
  };
  const collectStdout = createLineCollector(parseLine);
  const collectStderr = createLineCollector(parseLine);
...
async function main() {
  const toolkitResult = await runToolkitValidation();
  if (toolkitResult.code !== 0 || toolkitResult.signal) {
    if (toolkitResult.status) {
      console.error(`Toolkit validation failed with status: ${toolkitResult.status}`);
    }
    process.exitCode = toolkitResult.code ?? 1;
  ...
  if (toolkitResult.status) {
    console.log(`HARNESS_PREMERGE_TOOLKIT_STATUS=${toolkitResult.status}`);
  }
  if (result.status) {

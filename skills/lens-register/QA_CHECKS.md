...
8. `python3 tools/validation/validate_lens_register.py {deliverable_folder}` passes before the output is accepted.
...
| 9 | **Lens-specific no-item notes** | `NO_ITEMS` rows explain why that specific lens found no warranted edit; repeated boilerplate notes are invalid |
...
- `validate_lens_register.py` PASS

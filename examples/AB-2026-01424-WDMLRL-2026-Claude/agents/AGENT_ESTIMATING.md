...
**Alignment contract:**
- The **four deliverable documents** (`Datasheet.md`, `Specification.md`, `Guidance.md`, `Procedure.md`) are the primary source for identifying priceable items. ESTIMATING reads the engineering content to build a quantity takeoff before pricing.
- The active decomposition agent (**PROJECT_DECOMP** or **SOFTWARE_DECOMP**) provides stable nouns (Package IDs, Deliverable IDs) for WBS traceability and rollups.
- ESTIMATING produces **cost artifacts with traceability** back to document sources, pricing evidence, and WBS nouns, **without inventing facts**.
...
| **BLOCKING** | never |
| **PRIMARY_OUTPUTS** | immutable snapshot folder under `_Estimates/` containing `Items.csv`, `Summary.md`, `QA_Report.md`, `Source_Index.md`, logs, and (when possible) `Detail.csv` |
...
- **Deterministic outputs.** Given identical inputs (scope + documents + pricing sources + brief), the computed artifacts SHOULD be stable aside from timestamps and ordering normalizations.
...
Optional (context and traceability):
- `DECOMPOSITION_PATH`: path to the latest decomposition markdown (produced by PROJECT_DECOMP or SOFTWARE_DECOMP).
  - Used for WBS traceability (Package IDs, CBS hints) — not for line-item generation.
  - If not provided, WBS fields are populated best-effort from `_CONTEXT.md`.
...
#### Step 1 — Read the four documents
For each in-scope deliverable (resolved from `SCOPE`):
1. Read `_CONTEXT.md` to obtain deliverable ID, name, package, discipline, type.
2. Read the four production documents:
   - `Datasheet.md` — attributes, quantities, materials, conditions, construction details
   - `Specification.md` — requirements, standards, verification criteria
   - `Guidance.md` — principles, trade-offs, design rationale
   - `Procedure.md` — prerequisites, work steps, verification activities, records
3. If any of the four documents are missing for a deliverable, log `[WARNING] MISSING_DOCUMENT` per file and proceed with what exists.
4. (Optional) Read `DECOMPOSITION_PATH` if provided, to obtain Package IDs and CBS hints for WBS traceability.

#### Step 2 — Extract priceable items into `Items.csv`
From the engineering content read in Step 1, extract a structured list of priceable items:
1. For each deliverable, identify discrete items that require pricing:
   - Materials, components, assemblies, physical attributes (from Datasheet)
   - Compliance activities, testing, verification (from Specification)
   - Work activities, operations, installation steps (from Procedure)
   - Any other priceable scope explicitly stated in the documents
2. For each item, determine pricing mode:
   - `UNIT_RATE`: item has a measurable quantity and unit (e.g., 50 m² of formwork, 12 EA valves)
   - `LUMP_SUM`: item is priced as a whole (e.g., commissioning, mobilization, design review)
3. Record the source document and section for each item.
4. If a quantity or unit cannot be determined from the documents, record `TBD` and flag in Notes.
5. Write `Items.csv` to the snapshot folder (schema in STRUCTURE).

This CSV is a **standalone review artifact**. A human can verify the quantity takeoff independently before any pricing is applied.

#### Step 3 — Integrate pricing sources
1. Index `PRICE_SOURCES` into `Source_Index.md`:
   - each source file/folder path,
   - source type (quote/rate table/historical/parametric/allowance),
   - any parsing notes,
   - and what it can/cannot support.
2. For each row in `Items.csv`, match against pricing evidence and apply rates:
   - `QUOTE`: explicit quote line → `Method=QUOTE`
   ...
   - `PARAMETRIC`: compute from approved model + parameters → `Method=PARAMETRIC`
   - `ALLOWANCE`: use allowance tables/budgets → `Method=ALLOWANCE`
3. Enforce `BASIS_OF_ESTIMATE`, `ALLOW_MIXED_METHODS`, and `FALLBACK_POLICY`:
   - If fallback is not allowed and basis evidence is missing, set `Amount=TBD` and add a QA warning.
4. Write `Detail.csv` to the snapshot folder, carrying forward each item's `ItemID` for traceability back to the takeoff.
5. Populate provenance:
   - `SourceRef` must point to a pricing evidence file + section/row ID, OR to an entry in `Decision_Log.md` / `Assumptions_Log.md`.
   - If unknown, use `location TBD` and flag in QA.

If `PRICE_SOURCES` is empty or unusable:
- do not guess prices,
- write `Detail.csv` with `Amount=TBD` for all items and report per SPEC.

#### Step 4 — Produce rollups + matrices
...
- Produce `QA_Report.md` including:
  - schema validity (`Items.csv` and `Detail.csv` columns + enum validation),
  - quantity takeoff coverage (items extracted per deliverable, deliverables with missing documents),
  - pricing coverage (% of items priced vs `TBD`),
  - provenance completeness (% rows with non-TBD SourceRef in `Detail.csv`),
...
- `Items.csv` (quantity takeoff — priceable items extracted from the four documents)
- `Summary.md`
- `QA_Report.md`
- `Source_Index.md`
- `Decision_Log.md`
...
### `Items.csv` schema (quantity takeoff)

Columns (mandatory):
- `ItemID`
- `DeliverableID`
- `Description`
- `Qty` (numeric, or `TBD` if not determinable from documents)
- `Unit` (e.g., m², kg, hr, EA, LS)
- `PricingMode` (`UNIT_RATE` | `LUMP_SUM`)
- `SourceDoc` (`Datasheet` | `Specification` | `Guidance` | `Procedure`)
- `SourceSection` (section heading or reference within the source document)
- `Notes`
...
- `Run_Context.md`, `Items.csv`, `Summary.md`, `QA_Report.md`, and `Source_Index.md` exist.

### S5 — CSV schema integrity
- `Items.csv` is parseable, contains all required columns, every row traces to a source document and section, and `PricingMode` values are valid.
- `Detail.csv` (when produced) is parseable, contains all required columns, `Method` values are valid, and allowance/parametric convention is respected.
...
- Quantity takeoff (`Items.csv`) has been reviewed for completeness.
...
- Scope coverage is explicit (included/excluded counts and reasons).
